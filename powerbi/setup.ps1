...
.DESCRIPTION
    ...
    Supports three data-source modes:
      ...
      preloaded  - offline mode using CSVs downloaded by fetch-defender-data.ps1
                   (full per-device x CVE rows, no Fabric required)

.PARAMETER DataSource
    'api' (default), 'csv', or 'preloaded'.
    In csv/preloaded mode the fn_PaginatedGet function and API parameter are
    omitted; instead a CsvFolderPath parameter points to the folder with CSVs.

.PARAMETER CsvFolderPath
    Folder containing the CSV files (csv and preloaded modes).
...
.PARAMETER TenantId
    ...
    Use the tenant short name or Entra tenant ID GUID.
...
.EXAMPLE
    ...
    .\setup.ps1 -DataSource preloaded -TenantId "contoso" -CsvFolderPath ".\csv\contoso.onmicrosoft.com" -Open
